...
def validate(config: dict) -> None:
    ...
    date = config.get("date", {}).get("validation", {})
    if date.get("no_datetime_joins") and config.get("uses_datetime_join"):
        raise ValidationError("DateTime used as a join key (rule: no_datetime_joins)")
    if date.get("require_calendar") and not config.get("has_calendar_table"):
